...
export default function Search() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      {/* Search Header */}
      <div className="glass-card rounded-2xl border border-white/10 p-6 md:p-8 mb-8 relative overflow-hidden">
        ...
        <p className="text-surface-400 text-sm md:text-base max-w-xl mb-6">
        ...
        <div className="relative w-full max-w-2xl" ref={wrapperRef}>
          <div className="relative">
            ...
            <input
              ...
              className="input-field !rounded-full !px-12 !py-4 text-sm md:text-base"
            />
            <svg className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-surface-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4" />
            </svg>
            {query && (
              <button onClick={handleClear} className="absolute right-12 top-1/2 -translate-y-1/2 text-surface-500 hover:text-white transition">
                ✕
              </button>
            )}
          </div>
          ...
          {showSuggestions && (
            <div className="absolute z-50 w-full mt-2 glass-card border border-white/10 rounded-xl shadow-2xl overflow-hidden max-h-80 overflow-y-auto">
              {recentSearches.length > 0 && (
                <div className="p-2 border-b border-white/10">
                  <p className="text-xs text-surface-500 px-3 py-1 font-medium uppercase tracking-wider">Recent</p>
                  {recentSearches.map((term, i) => (
                    <button key={i} onClick={() => handleSelectRecent(term)} className="w-full text-left px-3 py-2 hover:bg-white/5 rounded-lg transition flex items-center gap-2 text-sm text-surface-300">
              ...
              {suggestions.length > 0 && (
                <div className="p-2">
                  <p className="text-xs text-surface-500 px-3 py-1 font-medium uppercase tracking-wider">Suggestions</p>
                  {suggestions.map((s, i) => (
                    <button key={i} onClick={() => handleSelectSuggestion(s.text)} className="w-full text-left px-3 py-2 hover:bg-white/5 rounded-lg transition flex items-center gap-2 text-sm text-surface-300">
                      <span className="text-surface-500">{s.type === 'artist' ? '🎤' : '🎵'}</span>
                      {s.text}
                      <span className="ml-auto text-xs text-surface-600">{s.type}</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        {!query && recentSearches.length === 0 && (
          <div className="mt-6">
            <p className="text-xs text-surface-500 mb-2 font-medium uppercase tracking-wider">Popular Searches</p>
            <div className="flex flex-wrap gap-2">
              {POPULAR_SEARCHES.map((term) => (
                <button
                  key={term}
                  onClick={() => { setQuery(term); performSearch(term); }}
                  className="px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs text-surface-400 hover:bg-white/10 hover:text-white transition"
                >
                  {term}
                </button>
              ))}
        ...
        {!query && recentSearches.length > 0 && (
          <div className="mt-6">
            <p className="text-xs text-surface-500 mb-2 font-medium uppercase tracking-wider">Recent Searches</p>
            <div className="flex flex-wrap gap-2">
      ...
      {searchMeta && !loading && query && (
        <div className="flex flex-wrap items-center gap-2 mb-6">
          <span className="text-sm text-surface-400">Results for &ldquo;{query}&rdquo;</span>
          {searchMeta.mood && (
          ...
          {!isFallback ? (
            ...
            <span className="px-2.5 py-1 rounded-full bg-slate-500/20 border border-slate-500/30 text-xs text-surface-400">
              Basic search
            </span>
          )}
        </div>
      ...
      {query && !loading && songs.length === 0 && (
        <div className="glass-card rounded-2xl p-12 md:p-16 text-center">
          ...
          <p className="text-surface-400 mb-2">We couldn&apos;t find any songs for &ldquo;{query}&rdquo;</p>
          <p className="text-surface-500 text-sm mb-6">Try different keywords or browse our popular searches above</p>
      ...
      {query && songs.length > 0 && (
        <div className="mb-6 flex items-center justify-between">
          ...
          <span className="text-sm text-surface-400">{songs.length} song{songs.length !== 1 ? 's' : ''}</span>
        </div>
      )}
      ...
      {!query && songs.length === 0 && !loading && (
        <div className="glass-card rounded-2xl p-12 md:p-16 text-center">
          ...
          <p className="text-surface-400">Search by mood, genre, artist, or song title. Try something like &ldquo;chill beats for coding&rdquo;</p>
        </div>
      )}
    </div>
  );
